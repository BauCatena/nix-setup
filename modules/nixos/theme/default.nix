...
let
  ...
  cfg = config.bautinix.theme;
in
{
  options.bautinix.theme = {
    ...
    icon = {
      ...
      package = mkOpt types.package pkgs.bibata-cursors "The package to use for the icon theme.";
    };
  ...
  config = mkIf cfg.enable {
    warnings = lib.mkIf (cfg.name == null) [
      "bautinix.theme is enabled, but no theme name (e.g. catppuccin, tokyonight, nord) is configured. You may have forgotten to enable a concrete theme module."
    ];
